...
type ChartToolResponse = ChartData;
...
export async function POST(req: NextRequest) {
    ...
    console.log(messages, fileData, model );
    return new Response(
        JSON.stringify({ error: "Messages array is required", messages: messages, fileData: fileData, model: model }),
        { status: 400 }
      );
  try {
    const { messages, fileData, model } = await req.json();
...
Focus on clear financial insights and let the visualization enhance understanding.`,
    ...
    console.log("✅ Anthropic API Response received:", response)
    ...
    //   ),
    //   contentTypes: response.content.map((c: { type: any }) => c.type),
    //   contentLength:
    //     response.content[0].type === "text"
    //       ? response.content[0].text.length
    //       : 0,
    //   toolOutput: response.content.find(
    //     (c: { type: string }) => c.type === "tool_use"
    //   )
    //     ? JSON.stringify(
